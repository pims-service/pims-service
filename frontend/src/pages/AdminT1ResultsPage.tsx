...
const AdminT1ResultsPage: React.FC = () => {
  ...
  const fetchSubmissions = async (page: number = 1) => {
    setLoading(true);
    try {
      const response = await questionnairesApi.getAdminT1Responses(page);
      ...
      console.error('Failed to fetch T1 follow-up responses', err);
      setError('Failed to load T1 follow-up data. Please verify administrative privileges.');
      setSubmissions([]);
    } finally {
      setLoading(false);
    }
  ...
  const handleViewDetail = async (id: string) => {
    try {
      const response = await questionnairesApi.getAdminT1Detail(id);
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="space-y-1">
            ...
            <h1 className="text-2xl md:text-3xl font-bold text-zinc-900 tracking-tight">
              1-Week Assessment Results
            </h1>
            <p className="text-zinc-500 text-sm">
              Psychometric results from completed 1-week assessments
            </p>
          </div>
...
export default AdminT1ResultsPage;
