...
const AdminT2ResultsPage: React.FC = () => {
  ...
  const fetchSubmissions = async (page: number = 1) => {
    setLoading(true);
    try {
      const response = await questionnairesApi.getAdminT2Responses(page);
      ...
      console.error('Failed to fetch T2 follow-up responses', err);
      setError('Failed to load T2 follow-up data. Please verify administrative privileges.');
      setSubmissions([]);
    } finally {
      setLoading(false);
    }
      ...
      pollInterval = setInterval(async () => {
        try {
          ...
          if (status === 'SUCCESS' && file_url) {
            ...
            link.setAttribute('download', `t2_results_${dateStr}.csv`);
  ...
  const handleViewDetail = async (id: string) => {
    try {
      const response = await questionnairesApi.getAdminT2Detail(id);
  ...
  const handleExport = async () => {
    try {
      ...
      const response = await questionnairesApi.triggerAdminT2Export(selectedGroup);
      setExportingId(response.data.task_id);
      setError(null);
    } catch (err) {
      console.error('Failed to export T2 follow-up data', err);
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="space-y-1">
            ...
            <h1 className="text-2xl md:text-3xl font-bold text-zinc-900 tracking-tight">
              3-Month Assessment Results
            </h1>
            <p className="text-zinc-500 text-sm">
              Psychometric results from completed 3-month assessments
            </p>
          </div>
...
export default AdminT2ResultsPage;
