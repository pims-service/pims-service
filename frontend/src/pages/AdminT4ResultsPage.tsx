...
const AdminT4ResultsPage: React.FC = () => {
  ...
  const fetchSubmissions = async (page: number = 1) => {
    setLoading(true);
    try {
      const response = await questionnairesApi.getAdminT4Responses(page);
      ...
      console.error('Failed to fetch T4 follow-up responses', err);
      setError('Failed to load T4 follow-up data. Please verify administrative privileges.');
      setSubmissions([]);
    } finally {
      setLoading(false);
    }
  ...
  const handleViewDetail = async (id: string) => {
    try {
      const response = await questionnairesApi.getAdminT4Detail(id);
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="space-y-1">
            ...
            <h1 className="text-2xl md:text-3xl font-bold text-zinc-900 tracking-tight">
              1-Year Assessment Results
            </h1>
            <p className="text-zinc-500 text-sm">
              Psychometric results from completed 1-year assessments
            </p>
          </div>
                  ...
                  <tr key={s.id} className="hover:bg-zinc-50 transition-colors">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        <div className="w-9 h-9 bg-indigo-50 flex items-center justify-center text-indigo-600 rounded-lg">
                          <User size={16} />
                        </div>
                        <div>
                    ...
                    <td className="px-6 py-4">
                      <span className="px-2 py-0.5 bg-indigo-50 text-xs font-medium text-indigo-700 border border-indigo-100 rounded-md">
...
export default AdminT4ResultsPage;
